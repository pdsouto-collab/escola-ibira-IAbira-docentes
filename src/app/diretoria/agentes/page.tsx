...
import { ArrowLeft, Save, RefreshCw, AlertCircle, CheckCircle, Bot, MessageSquare, PenTool } from 'lucide-react';
    ...
    <div className="flex h-screen bg-[#f2efe9] font-sans">
      ...
      <div className="w-64 bg-[#4a5d4e] text-white flex flex-col p-6 shadow-xl z-10">
        <div className="mb-10 font-bold text-xl tracking-tight text-[#e8a375]">
          Escola Ibirá
          <span className="block text-sm font-normal text-[#8fb39c]">Treinamento da IA</span>
        </div>
        <nav className="flex flex-col gap-2">
          <div 
            className="p-3 hover:bg-[#394a3d] rounded-lg text-sm cursor-pointer opacity-70 flex items-center gap-2 transition-all"
            onClick={() => window.location.href = '/diretoria'}
          >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b p-4 flex justify-between items-center shadow-sm">
          <div className="flex items-center gap-3">
            <button 
              onClick={() => window.location.href = '/diretoria'} 
              className="text-[#4a5d4e] hover:text-[#e8a375] transition-colors"
            >
              <ArrowLeft size={24} />
            </button>
            <h1 className="text-2xl font-bold text-[#2A4B3A]">Diretrizes dos Agentes</h1>
          </div>
          <Button 
            className="bg-white border border-[#4a5d4e] text-[#4a5d4e] hover:bg-[#f2efe9] transition-all shadow-sm" 
            onClick={() => window.location.href = '/diretoria'}
          >
            Ir para Painel da Diretoria
          </Button>
        </header>
