...
import { CheckCircle, AlertTriangle, ArrowRight, MessageSquare, Loader2 } from "lucide-react";
...
function DirectorDashboardContent() {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      default:
        return (
          <main className="flex-1 p-10 overflow-y-auto flex gap-10 h-[calc(100vh-80px)]">
            {/* LISTA */}
  ...
  return (
    <div className="flex h-screen bg-[#f2efe9] font-sans">
      {/* SIDEBAR */}
      <div className="w-80 bg-[#4a5d4e] text-white flex flex-col p-5 shadow-xl z-10 shrink-0">
        ...
        <nav className="flex flex-col gap-2">
          <button 
            ...
            onClick={() => handleTabChange("aprovações")}
          >
          ...
          <button 
            ...
            onClick={() => handleTabChange("agentes")}
          >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b p-4 flex justify-between items-center shadow-sm z-10 shrink-0">
          <h1 className="text-2xl font-bold text-[#2A4B3A]">{getHeaderTitle()}</h1>
          <div className="flex gap-3 flex-wrap items-center">
            <Button 
              className="bg-white border border-[#4a5d4e] text-[#4a5d4e] hover:bg-[#f2efe9] transition-all" 
